...
typedef struct rdm_BridgingBr_s {   // RDM_OID_BRIDGING_BR 
	...
	char X_ZXYEL_MacLearningIfaces[1057];
} rdm_BridgingBr_t;
...
typedef struct rdm_PppIface_s {   // RDM_OID_PPP_IFACE 
	...
	uint32_t X_ZYXEL_PCPMode;
	char X_ZYXEL_PCPServer[257];
...
typedef struct rdm_IpIface_s {   // RDM_OID_IP_IFACE 
	...
	bool X_ZYXEL_v6DefaultGatewayIface;
	int X_ZYXEL_ConnectionId;
	uint32_t X_ZYXEL_IPR2_MARKING;
	...
	bool X_ZYXEL_IPv6Unnumbered_Model;
...
typedef struct rdm_NatPortControl_s {   // RDM_OID_NAT_PORT_CONTROL 
	bool Enable;
	char ExternalIPAddress[257];
	char InternalIPAddress[46];
	char InternalCpeIPAddress[257];
	char PCPServer[257];
	uint32_t RequiredInternalPort;
	uint32_t RequiredPublicPort;
	uint32_t AssignedPublicPort;
	char Protocol[4];
	uint32_t ResultCode;
	uint32_t Nonce1;
	uint32_t Nonce2;
	uint32_t Nonce3;
	char RecvLifetimeEnd[257];
	char LifetimeRenewS[257];
	uint32_t AllowPCPPortProposal;
} rdm_NatPortControl_t;
